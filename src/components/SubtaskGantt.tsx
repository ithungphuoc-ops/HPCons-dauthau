...
// ===== KHÔNG TỰ TÍNH KẾ HOẠCH VIỆC CON Ở ĐÂY NỮA (chị Trâm báo 15/09/2026) =====
// Toàn bộ phép tính mốc kế hoạch đã chuyển sang src/utils/keHoachViecCon.ts để App.tsx dùng CHUNG
// một thước đo — xem lý do đầy đủ ở đầu file đó. Re-export để mọi chỗ đang import từ SubtaskGantt
// vẫn chạy như cũ.
import {
  DAY_MS,
  DEFAULT_TASK_DAYS,
  ...
  ngayCuoiLamViec,
  demSoNgay,
  khoangKeHoachViecCon,
} from '../utils/keHoachViecCon';

export { DEFAULT_TASK_DAYS, lamTronNuaNgay, ngayCuoiLamViec, demSoNgay, khoangKeHoachViecCon };
...
export default function SubtaskGantt({ tasks, staff, projectStartDate, canEdit, isBOOD = false, hideFooter = false, vongHienTai = 1, thuVienTen = [], onChange }: SubtaskGanttProps) {
  ...
  // ===== CHIA ĐỀU LẠI TOÀN BỘ (chị Trâm chốt 19/09/2026) =====
  // "Trong mục điều chỉnh phân bổ mà có thêm công việc con, cho chị nút phân chia tỉ trọng cho
  //  nhanh em, để vướng rule 100% cứ chia tay lại phải sửa lại mấy công việc con bên trên rắc rối."
  //
  // Khác `chiaDeuTiTrong` ở trên: hàm kia GIỮ NGUYÊN việc đã gõ số và chỉ chia phần còn thiếu, nên
  // khi bộ việc đã đủ 100% mà thêm việc mới thì việc mới nhận 0% và người dùng vẫn phải sửa tay
  // từng việc phía trên để nhường chỗ. Hàm này chia lại ĐỀU cho MỌI việc cấp 1 của vòng — thêm
  // việc thứ 4 vào bộ 3 việc là bấm một cái ra 25/25/25/25.
  // Phần dư do làm tròn dồn vào việc ĐẦU TIÊN để tổng khớp đúng 100%, không lẻ 99% hay 101%.
  const chiaDeuTatCa = () => {
    const cungVong = tasks.filter(t => vongCua(t) === vongHienTai);
    if (cungVong.length === 0) return;
    const moiViec = Math.floor(100 / cungVong.length);
    const du = 100 - moiViec * cungVong.length;
    const idNhan: Record<string, number> = {};
    cungVong.forEach((t, i) => { idNhan[t.id] = moiViec + (i === 0 ? du : 0); });
    guiThayDoi(tasks.map(t => (idNhan[t.id] !== undefined ? { ...t, weight: idNhan[t.id] } : t)));
  };
  ...
  // Số việc cấp 1 của VÒNG ĐANG CHẠY — nút "Chia đều" chỉ có nghĩa khi có từ 2 việc trở lên.
  const soViecVongNay = tasks.filter(t => vongCua(t) === vongHienTai).length;
  ...
  return (
    <div className="bg-white dark:bg-dark-card border border-slate-200/60 dark:border-slate-800 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-100 dark:border-slate-800">
        ...
        {/* CHIA ĐỀU đặt ngay cạnh "Thêm việc con": bảng "Phân bổ lại việc con" ẩn chân bảng
            (hideFooter) nên nút chia tỉ trọng dưới đó không tới tay người dùng — mà đó lại đúng
            là chỗ hay phải thêm việc giữa chừng nhất. */}
        {canEdit && soViecVongNay >= 2 && (
          <button
            type="button"
            onClick={chiaDeuTatCa}
            title={`Chia đều 100% cho ${soViecVongNay} việc con của vòng này (mỗi việc ${Math.floor(100 / soViecVongNay)}%). Dùng khi vừa thêm việc mới mà không muốn sửa tay từng dòng.`}
            className="text-[10px] font-black bg-brand-primary/10 text-brand-primary dark:text-brand-primary-300 hover:bg-brand-primary/20 px-2 py-1 rounded-lg flex items-center gap-1 transition-colors shrink-0 border border-brand-primary/25"
          >
            ⚖ Chia đều
          </button>
        )}
